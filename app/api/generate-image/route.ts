...
async function fetchWithRetry(url: string, options: RequestInit, retries = 0): Promise<Response> {
  ...
  try {
    ...
    if (!response.ok && (response.status === 429 || response.status >= 500) && retries < MAX_RETRIES) {
      ...
      console.warn(`Upstream error ${response.status} for ${url}. Retrying in ${waitTime}ms... (Attempt ${retries + 1}/${MAX_RETRIES})`);
    ...
    if (retries < MAX_RETRIES) {
      const waitTime = RETRY_DELAY_BASE * Math.pow(2, retries);
      console.warn(`Fetch network error: ${error}. Retrying in ${waitTime}ms... (Attempt ${retries + 1}/${MAX_RETRIES})`);
      await delay(waitTime);
      return fetchWithRetry(url, options, retries + 1);
    }
    throw error;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Upstream Failure [${response.status}] after ${duration}ms:`, {
